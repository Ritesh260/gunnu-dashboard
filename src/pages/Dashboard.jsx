...
function Dashboard() {
  ...
  const getItemPrice = (item) => {
    ...
    if (item?.price && typeof item.price === "object") {
      return Number(item.price.full ?? item.price.half ?? 0) || 0;
    }
    ...
    if (item?.halfPrice !== undefined && item?.halfPrice !== null) {
      return Number(item.halfPrice) || 0;
    }
  ...
  const nonVegCount = items.filter(
    (item) => item.type === "non-veg" || item.type === "nonveg"
  ).length;
    ...
    <div className="min-h-screen bg-gray-100 dark:bg-gray-950 text-black dark:text-white transition-all duration-300 px-3 sm:px-5 lg:px-8 py-4 sm:py-6 overflow-x-hidden">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4 sm:gap-5">
        ...
        <div className="bg-white dark:bg-gray-900 rounded-2xl p-5 sm:p-6 border border-gray-200 dark:border-gray-800 shadow-sm">
          ...
          <p className="text-3xl sm:text-4xl font-bold mt-4">{items.length}</p>
        </div>
        ...
        <div className="bg-white dark:bg-gray-900 rounded-2xl p-5 sm:p-6 border border-gray-200 dark:border-gray-800 shadow-sm">
          ...
          <p className="text-3xl sm:text-4xl font-bold mt-4">{vegCount}</p>
        </div>
        ...
        <div className="bg-white dark:bg-gray-900 rounded-2xl p-5 sm:p-6 border border-gray-200 dark:border-gray-800 shadow-sm">
          ...
          <p className="text-3xl sm:text-4xl font-bold mt-4">{nonVegCount}</p>
        </div>
